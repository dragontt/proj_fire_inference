...
def parse_args(argv):
    parser = argparse.ArgumentParser(description="Combine and sort the monomers and gapped dimers inferred in zinc cluster family")
    parser.add_argument('-m', '--monomer', dest='monomer', help='monomer fire summary')
    parser.add_argument('-d', '--dimer', dest='dimer', help='gapped dimer fire summary')
    parser.add_argument('-o', '--output_fn', dest='output_fn', type=str, default='./combined_fire_summary')
...
def main(argv):
    ...
    dimLines = open(parsed.dimer, "r").readlines()
    for line in dimLines:
        if line.strip():
            ...
            rankDict[motif] = 0
    if os.path.isfile(parsed.monomer):
        monoLines = open(parsed.monomer, "r").readlines()
        for line in monoLines:
            if line.strip():
                lineSplit = line.split()
                motif = lineSplit[0]
                miDict[motif] = float(lineSplit[2])
                zsDict[motif] = float(lineSplit[4])
                rbDict[motif] = int(lineSplit[5].split("/")[0])
                gapDict[motif] = "m"
                rankDict[motif] = 0
