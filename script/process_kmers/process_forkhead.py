...
def parse_args(argv):
    parser = argparse.ArgumentParser(description="Constrain k-mers in forkhead family")
...
def main(argv):
    ...
    for kmer in kmersList:
        kmer = kmer.split('\n')[0]
        # forkhead core sequence: [GA][TC]AAA[TC]A, GA[TC]GC
        dbdSeqList = ['GATGC', 'GACGC', 'GCATC', 'GCGTC', 
        'GTAAATA', 'GTAAACA', 'GTAAATA', 'GCAAACA', 'ATAAATA', 'ATAAACA', 'ATAAATA', 'ACAAACA',
        'TATTTAC', 'TGTTTAC', 'TATTTAC', 'TGTTTGC', 'TATTTAT', 'TGTTTAT', 'TATTTAT', 'TGTTTGT']
