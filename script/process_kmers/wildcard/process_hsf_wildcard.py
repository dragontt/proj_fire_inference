...
def parse_args(argv):
    parser = argparse.ArgumentParser(description="Constrain k-mers in heat shock factor (HSF) family, allowing 1 wildcard position.")
...
def main(argv):
    ...
    # contain AGAA and its reverse complement TTCT
    k_hsf0 = 'AGAA'
    k_hsf1 = 'TTCT'
    for kmer in kmersList:
        kmer = kmer.split('\n')[0]
        newKmersList = filterkmers(kmer, k_hsf0, newKmersList)
        newKmersList = filterkmers(kmer, k_hsf1, newKmersList)
