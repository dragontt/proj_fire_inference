...
def counter(data, table, sign_type, fam_dict, sign_dict):
    for key_d, val_d in data.iteritems():
        if val_d['kmer'] == '6' or val_d['kmer'] == '7':
            col_num = fam_dict[val_d['fam']]*2
            row_num = sign_dict[val_d[sign_type]]
            table[col_num][row_num] += 1
        elif val_d['kmer'] == '8':
            col_num = fam_dict[val_d['fam']]*2 + 1
            row_num = sign_dict[val_d[sign_type]]
            table[col_num][row_num] += 1
    return table

def sumcols(table, num_families, num_signs):
    for i in range(num_families*2):
        for j in range(num_signs):
            if i%2 == 0:
                table[num_families*2][j] += table[i][j]
            else:
                table[num_families*2+1][j] += table[i][j]
    return table
...
def writetable(writer, table, num_families, num_signs, fam_dict):
    ...
    for i in range(num_families*2):
        ...
        for key, val in fam_dict.iteritems():
            if i/2 == val:
                if key == "znclus":
                    writer.write("%s_k6\n" % (key)) if i%2 == 0 else writer.write("%s_k8\n" % (key))
                else:
                    writer.write("%s_k7\n" % (key)) if i%2 == 0 else writer.write("%s_k8\n" % (key))
    for item in table[num_families*2]: 
        writer.write("%.3f\t" % (item)) 
    writer.write("all_k6/7\n")
    for item in table[num_families*2+1]:
        writer.write("%.3f\t" % (item)) 
    writer.write("all_k8\n")
...
def main(argv):
    ...
    # count table
    linesFam = open(parsed.all_fam_dir, "r").readlines()
    num_signs = 4
    sign_dict = {'+1': 0, '0': 1, '-1': 2, 'N/A': 3}
    num_families = 0
    fam_dict = {}
    for i, lineFam in enumerate(linesFam):
        fam_dict[lineFam.split('\n')[0]] = i
    num_families = i+1

    table_count_fire = numpy.zeros(shape=(num_families*2+2, num_signs))
    table_count_tomtom = numpy.zeros(shape=(num_families*2+2, num_signs))
    table_count_combined = numpy.zeros(shape=(num_families*2+2, num_signs))
